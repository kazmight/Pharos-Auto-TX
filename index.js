...
const contractAddress = '0x1a4de519154ae51200b0ad7c90f7fac75547888a'; // Likely a router/aggregator
...
const erc20Abi = [
    ...
    'function deposit() public payable', // For WETH/WPHRS
    'function withdraw(uint256 wad) public', // For WETH/WPHRS
];
...
const MAX_RETRIES = 5; // Max attempts for retrying RPC calls
const RETRY_DELAY_MS = 5000; // Starting delay for retries (5 seconds)
    ...
    try {
        ...
        if (allowance < required) {
            ...
            try {
                ...
                logger.error(`Gas estimation for approval failed: ${error.message}`);
                return false;
            }

            const feeData = await wallet.provider.getFeeData();
            const gasPrice = feeData.gasPrice || ethers.parseUnits('1', 'gwei'); // Fallback if gasPrice is null/undefined
            ...
            logger.loading('Approval transaction sent, waiting for confirmation...');
            let receipt;
            for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                try {
                    ...
                    if (waitError.code === 'UNKNOWN_ERROR' && waitError.error && waitError.error.message.includes('Unable to complete the request')) {
                        logger.warn(`Attempt ${attempt + 1}/${MAX_RETRIES}: RPC error waiting for approval confirmation. Retrying in ${RETRY_DELAY_MS * (attempt + 1)}ms...`);
            ...
            if (receipt) {
                ...
                logger.error(`Approval failed after ${MAX_RETRIES} attempts.`);
                return false;
            }
        }
...
const getMulticallData = (pair, amount, walletAddress) => {
    try {
        ...
        // This encoding assumes the multicall contract expects specific parameters for a swap.
        // The '0x04e45aaf' is a function selector for a specific swap function.
        const data = ethers.AbiCoder.defaultAbiCoder().encode(
            ['address', 'address', 'uint256', 'address', 'uint256', 'uint256', 'uint256'],
            [
                tokens[pair.from],
                tokens[pair.to],
                500, // Likely a fee parameter, e.g., 0.05%
                walletAddress,
                scaledAmount,
                0, // amountOutMin (min amount to receive, set to 0 for simplicity/testnet)
                0, // sqrtPriceLimitX96 (Uniswap V3 specific, set to 0)
            ]
        );
    ...
    try {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                if (waitError.code === 'UNKNOWN_ERROR' && waitError.error && waitError.error.message.includes('Unable to complete the request')) {
                    logger.warn(`Attempt ${attempt + 1}/${MAX_RETRIES}: RPC error waiting for swap confirmation. Retrying in ${RETRY_DELAY_MS * (attempt + 1)}ms...`);
                    await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS * (attempt + 1)));
                } else {
                    throw waitError;
                }
        ...
        if (receipt) {
            ...
            logger.error(`Swap ${index + 1} failed after ${MAX_RETRIES} attempts.`);
        }
...
const transferPHRS = async (wallet, provider, index) => {
    try {
        const amount = 0.000001; // Small fixed amount
        const randomWallet = ethers.Wallet.createRandom(); // Send to a new random address
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                if (waitError.code === 'UNKNOWN_ERROR' && waitError.error && waitError.error.message.includes('Unable to complete the request')) {
                    logger.warn(`Attempt ${attempt + 1}/${MAX_RETRIES}: RPC error waiting for transfer confirmation. Retrying in ${RETRY_DELAY_MS * (attempt + 1)}ms...`);
                    await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS * (attempt + 1)));
                } else {
                    throw waitError;
                }
        ...
        if (receipt) {
            ...
            logger.error(`Transfer ${index + 1} failed after ${MAX_RETRIES} attempts.`);
        }
    ...
    try {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                if (waitError.code === 'UNKNOWN_ERROR' && waitError.error && waitError.error.message.includes('Unable to complete the request')) {
                    logger.warn(`Attempt ${attempt + 1}/${MAX_RETRIES}: RPC error waiting for wrap confirmation. Retrying in ${RETRY_DELAY_MS * (attempt + 1)}ms...`);
                    await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS * (attempt + 1)));
                } else {
                    throw waitError;
                }
        ...
        if (receipt) {
            ...
            logger.error(`Wrap ${index + 1} failed after ${MAX_RETRIES} attempts.`);
        }
...
const performCheckIn = async (wallet) => {
    try {
        ...
        if (checkInData.code === 0) {
            ...
            return jwt; // Still return JWT even if already checked in
        }
    } catch (error) {
    ...
    try {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                if (waitError.code === 'UNKNOWN_ERROR' && waitError.error && waitError.error.message.includes('Unable to complete the request')) {
                    logger.warn(`Attempt ${attempt + 1}/${MAX_RETRIES}: RPC error waiting for LP confirmation. Retrying in ${RETRY_DELAY_MS * (attempt + 1)}ms...`);
                    await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS * (attempt + 1)));
                } else {
                    throw waitError;
                }
        ...
        if (receipt) {
            ...
            logger.error(`Liquidity Add ${index + 1} failed after ${MAX_RETRIES} attempts.`);
        }
...
const main = async () => {
    ...
    if (!privateKeys.length) {
        logger.error('No private keys found in .env. Please set PRIVATE_KEY_1, PRIVATE_KEY_2, etc.');
        return;
    }
    ...
    while (true) {
        for (const privateKey of privateKeys) {
            ...
            // Perform initial login and faucet claim (these are not within a loop, good)
            await claimFaucet(wallet);
            await new Promise(resolve => setTimeout(resolve, Math.random() * 5000 + 2000)); // Delay after faucet claim

            const jwt = await performCheckIn(wallet);
            await new Promise(resolve => setTimeout(resolve, Math.random() * 5000 + 2000)); // Delay after check-in
            ...
            for (let i = 0; i < numTransfers; i++) {
                ...
                await new Promise(resolve => setTimeout(resolve, Math.random() * (maxDelayMs - minDelayMs) + minDelayMs));
            }
