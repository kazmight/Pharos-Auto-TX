...
// *** MODIFICATION: Added verificationDelay parameter ***
const performSwap = async (wallet, provider, index, jwt, proxy, verificationDelay) => {
  try {
...
    logger.step(`Explorer: https://testnet.pharosscan.xyz/tx/${receipt.hash}`);
    ...
    // *** MODIFICATION: Using manual delay ***
    logger.loading(`Waiting ${verificationDelay} seconds for the Pharos API to see the transaction before verification...`);
...
// *** MODIFICATION: Added verificationDelay parameter ***
const transferPHRS = async (wallet, provider, index, jwt, proxy, verificationDelay) => {
  try {
...
// *** MODIFICATION: Added verificationDelay parameter ***
const wrapPHRS = async (wallet, provider, index, jwt, proxy, verificationDelay) => {
  try {
...
// *** MODIFICATION: Added verificationDelay parameter ***
const addLiquidity = async (wallet, provider, index, jwt, proxy, verificationDelay) => {
  try {
...
const getManualDelay = (promptText, defaultValue) => {
    let delay = prompt(promptText);
    const seconds = parseInt(delay, 10);
    if (isNaN(seconds) || seconds <= 0) {
        logger.warn(`Invalid input, using default of ${defaultValue} seconds.`);
        return defaultValue;
    }
    return seconds;
}
...
  logger.banner();
  ...
  // *** MODIFICATION: Get all delays from user input ***
  const delayBetweenTx = getManualDelay('Enter delay BETWEEN each transaction in seconds (e.g., 20): ', 20);
  const verificationDelay = getManualDelay('Enter delay for API VERIFICATION in seconds (e.g., 10): ', 10);
  const cycleDelayMinutes = getManualDelay('Enter delay between CYCLES in minutes (e.g., 30): ', 30);
  
  logger.info(`Delay between transactions set to ${delayBetweenTx} seconds.`);
  logger.info(`Delay for API verification set to ${verificationDelay} seconds.`);
  logger.info(`Delay between cycles set to ${cycleDelayMinutes} minutes.`);


  const proxies = loadProxies();
  const privateKeys = [process.env.PRIVATE_KEY_1, process.env.PRIVATE_KEY_2, process.env.PRIVATE_KEY_3, process.env.PRIVATE_KEY_4, process.env.PRIVATE_KEY_5, process.env.PRIVATE_KEY_6, process.env.PRIVATE_KEY_7, process.env.PRIVATE_KEY_8, process.env.PRIVATE_KEY_9].filter(pk => pk);
...
      }
      ...
      const waitBetweenTxs = () => {
          logger.loading(`Waiting for ${delayBetweenTx}s before the next transaction...`);
          return new Promise(resolve => setTimeout(resolve, delayBetweenTx * 1000));
      }
...
      for (let i = 0; i < numTransfers; i++) {
        // *** MODIFICATION: Pass delay to function ***
        await transferPHRS(wallet, provider, i, jwt, proxy, verificationDelay);
        await waitBetweenTxs();
      }
...
      for (let i = 0; i < numWraps; i++) {
        // *** MODIFICATION: Pass delay to function ***
        await wrapPHRS(wallet, provider, i, jwt, proxy, verificationDelay);
        await waitBetweenTxs();
      }
...
      for (let i = 0; i < numSwaps; i++) {
        // *** MODIFICATION: Pass delay to function ***
        await performSwap(wallet, provider, i, jwt, proxy, verificationDelay);
        await waitBetweenTxs();
      }
...
      for (let i = 0; i < numLPs; i++) {
        // *** MODIFICATION: Pass delay to function ***
        await addLiquidity(wallet, provider, i, jwt, proxy, verificationDelay);
        await waitBetweenTxs();
      }
    }

    logger.success('All actions completed for all wallets!');
    await countdown(cycleDelayMinutes);
  }
};
