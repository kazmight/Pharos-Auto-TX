...
const waitForTransactionWithRetry = async (provider, txHash, maxRetries = 5, baseDelayMs = 1000) => {
  ...
  while (retries < maxRetries) {
    try {
      ...
      if (error.code === -32008) { // Specific error code for transaction not found on some RPCs
...
const getMulticallData = (pair, amount, walletAddress) => {
  try {
    ...
    // This is for exactInputSingle from Uniswap V3 Router
    // 0x04e45aaf is the function selector for exactInputSingle
...
const main = async () => {
  ...
  // Get all delays from user input
  ...
  // Filter out undefined/null private keys from process.env
  const privateKeys = [
  ...
  ].filter(pk => pk); // Filters out null, undefined, and empty strings
